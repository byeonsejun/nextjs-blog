...
export default function ContactForm() {
  ...
  const onSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    .catch(() => {
      setBanner({message: '메일 전송에 실패하였습니다. 다시 시도해 주세요.', state: 'error'});
    })
    .finally(() => {
  ...
  <form onSubmit={onSubmit} className='w-full flex flex-col gap-2 my-4 p-4 bg-slate-700 rounded-lg'>
    <label htmlFor='from' className='text-white font-semibold'>Your Email</label>
    <input 
      type="email" 
      id='from'
      name='from' 
